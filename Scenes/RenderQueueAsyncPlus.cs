...
public class RenderQueueAsyncPlus
{
    ...
    private void Execute(RenderJobPlus job)
    {
        ...
        int genIdAtStart = chunk.mGenerationId;
        ...
        if (data != null && chunk.mGenerationId == genIdAtStart)
            mResultsLOD.Enqueue((chunk, data, genIdAtStart));
    ...
    // APPLY (MAIN THREAD)
    // =========================================================
    public void Apply(Chunk chunk, MeshData data, int expectedGenId)
    {
        if (chunk.mViewGO == null) return;
        if (chunk.mGenerationId != expectedGenId) return;
